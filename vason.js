...
class Vason {
    ...
    _isValidNode(val) {
        ...
        if (val && typeof val === "object" && (Array.isArray(val._array) || Array.isArray(val._pair))) return true;
        return false;
    }
    ...
    get(key) {
        ...
        if (typeof key === "number") {
            if (this.data && this.data._array && key >= 0 && key < this.data._array.length) {
    ...
    set(index, val) {
        ...
        if (!this._isValidNode(val)) {
            throw new Error("Value must be a string, { _array: [...] }, or { _pair: [...] }");
        }
        ...
        if (this.data && this.data._array) {
            ...
            if (index !== 0 && index !== 1) throw new Error("Pair index must be 0 (key) or 1 (value).");
