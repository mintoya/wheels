...
struct {
  ...
  for (int i = _countof(res._); i > 0; i--)
    res._[_countof(res._) - i] = (typeof(*res._)){
        .data = (len >> ((i - 1) * 7)) & 0x7f,
        .hasNext = !!(i - 1),
    };
  return res;
...
fptr stringList_append(stringList *sl, fptr ptr) {
  ...
  ptrdiff_t place = 0;
  vlength *vl = vl_._;
  usize width_length = _countof(vl_._);
  ...
  if (potentialFree) {
    ...
    if (sl->len + _countof(vl_._) + ptr.width > sl->cap) {
      sl->ptr =
          aRealloc(
              allocator,
              sl->ptr,
              sl->len + _countof(vl_._) + ptr.width + sizeof(u64_toVlen(0))
          );
    }
...
fptr stringList_set(stringList *sl, List_index_t idx, fptr ptr) {
  ...
  if (lastLen_length < ptr.width) {
    ...
    usize width_length = _countof(newlen._);
