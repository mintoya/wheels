...
//
// basic closure function wrapper
//
...
//
  ...
  #define _fn_wrap_def(name, in, out, PtrType, RetType, WrapBody, ...)                                                      \
    out name(thrdfn_argItems in){__VA_ARGS__ closure_fn_return_if_nothing(out)} RetType name##_wrap(void *_thrd_item_ptr) { \
      PtrType argstruct = (PtrType)_thrd_item_ptr;                                                                          \
      argstruct->result = name(thrdfn_argItemsT in);                                                                        \
      WrapBody                                                                                                              \
    }
  #define _fn_wrap_dec(name, in, out, PtrType, RetType) \
    out name(thrdfn_argItems in);                       \
    RetType name##_wrap(void *_thrd_item_ptr);

  #define thrdfn_dec(name, in, out)       \
    ...
    _fn_wrap_dec(name, in, out, name##_future, int)
  ...
  #define thrdfn(name, in, out, ...)      \
    ...
    _fn_wrap_def(name, in, out, name##_future, int, return 1;, __VA_ARGS__)
  ...
  #define thrdfn_await(alloc, f) ({                       \
    typeof(f) f_r = f;                                    \
    defer { aFree(alloc, f_r, sizeof(*f_r)); };           \
...
thrdfn_dec(tpool_worker, ((tpool_single_t, pool)), int);

  #define poolfn_dec(name, in, out) \
    typedef struct name##_future_ { \
      struct {                      \
        thrdfn_structItems in       \
      } args;                       \
      out result;                   \
      _Atomic(bool) done[1];        \
        ...
        *name##_future;             \
    _fn_wrap_dec(name, in, out, name##_future, void)
  ...
  #define poolfn(name, in, out, ...) \
    ...
    _fn_wrap_def(name, in, out, name##_future, void, atomic_store(argstruct->done, true);, __VA_ARGS__)

  #define pool_handle_object(FType) \
    struct {                        \
      FType future[1];              \
      tpoolNode_t node[1];          \
    }
  ...
  #define poolfn_call(pool, fname, argss) ({                               \
    typedef pool_handle_object(typeof(*(fname##_future)NULL)) pho_t;       \
    var_ alloc = tpool_allocator(pool);                                    \
    var_ pitem = aCreate(alloc, pho_t);                                    \
    var_ f = pitem->future;                                                \
    (void)sizeof(fname argss);                                             \
    f->args = (typeof(f->args)){REM_PAREN argss};                          \
    f->done[0] = false;                                                    \
  ...
  #define poolfn_await(pool, f) ({                  \
    var_ alloc = tpool_allocator(pool);             \
    typedef pool_handle_object(typeof(*(f))) pho_t; \
    typeof(f) f_r = f;                              \
                                                    \
    _tpool_wait_loop(pool, f_r->done);              \
                                                    \
    typeof(f_r->result) res = f_r->result;          \
    aFree(alloc, f_r, sizeof(pho_t));               \
    res;                                            \
  })

void _tpool_enqueue(tpool_single_t pool, tpoolNode_t *nodeMem, basic_closure_t fn);
void _tpool_wait_loop(tpool_single_t pool, _Atomic(bool) *done_flag);

mutex(tpool, mutex_recursive) * tpool_init(AllocatorV alloc);
...
thrdfn(tpool_worker, ((tpool_single_t, pool)), int, {
  while (1) {
    if (tpool_doSingle(pool))
      continue;
